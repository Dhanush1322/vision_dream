...
function InactiveUser() {
    ...
    const fetchUsers = async () => {
        try {
            const response = await fetch(`https://goldfish-app-nczbc.ondigitalocean.app/api/admin/user/all?page=${currentPage}&limit=${itemsPerPage}&filter=inactive`, {
                method: "GET",
                headers: {
            ...
            if (response.ok) {
                setUsers(data.result?.users?.docs || []);  // Ensure `users` is always an array
                setTotalPages(data.result?.users?.totalPages || 1); // Handle undefined `totalPages`
            } else {
                console.error("Failed to fetch users:", data.message);
                setUsers([]); // Prevents undefined issues
            }
        } catch (error) {
                        ...
                        <table className="user-table">
                            ...
                            <tbody>
                                {users.length > 0 ? (
                                    ...
                                    <tr>
                                        <td colSpan="8" className="no-data">No inactive users found</td>
                                    </tr>
                                )}
                            </tbody>
                        </table>
